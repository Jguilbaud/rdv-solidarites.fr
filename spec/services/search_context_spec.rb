...
describe SearchContext, type: :service do
  ...
  describe "#current_step" do
    context "with a suivi and only one motif existing" do
      it "returns service selection" do
        search = described_class.new(user, { suivi: "1" })
        expect(search.current_step).to eq(:service_selection)
      end

      it "returns motif selection" do
        service = create(:service)
        search = described_class.new(user, { suivi: "1", service_id: service.id })
